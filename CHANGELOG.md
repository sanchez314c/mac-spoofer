...
## [1.2.4] - 2026-03-14 - Forensic Security Audit & Hardening

### Security (CRITICAL)
- **`src/auth-handler.js`** — Fixed command injection in macOS osascript execution path: command is now shell-quoted and backslashes/double-quotes are escaped before embedding in the AppleScript `do shell script "..."` string, preventing boundary breakout
- **`src/auth-handler.js`** — Fixed PowerShell injection in Windows elevation path: `command` is now escaped via `psEscape()` (double-quote doubling); changed `shell: true` to `shell: false` on PowerShell spawn
- **`src/main.js`** — Added strict input validation in `spoof-mac` and `restore-mac` IPC handlers: interface name validated against `^[\w\-.]{1,64}$`, MAC address validated against full hex pattern before passing to Python subprocess

### Security (HIGH)
- **`src/app.js`** — Added `escHtml()` helper; applied to all `innerHTML` insertions in `renderInterfaces`, `renderStatus`, `updateSelectedInterfaceDisplay`, and `showToast` — prevents XSS from crafted interface names or MAC addresses in Python script output
- **`src/main.js`** — `open-external` IPC handler now validates URL protocol (allows `https:` and `http:` only), preventing `file://`, `app://`, and `javascript:` URI injection

### Security (MEDIUM)
- **`src/universal_mac_spoof.py`** — Replaced bare `except:` clauses with `except Exception as e:` throughout `get_interfaces()`, eliminating silent failure swallowing
- **`src/universal_mac_spoof.py`** — Replaced `cat /sys/class/net/{interface}/address` subprocess with direct Python file read

### Fixed
- **`src/preload.js`** — Renamed `interface` parameter (reserved word in strict JS/TS) to `interfaceName` in `restoreMac`
- **`scripts/run-linux-source.sh`** — Removed duplicate `print_error` and `command_exists` function definitions; added missing `CYAN` color variable declaration

### Dependencies
- Ran `npm audit fix` — patched 19 of 22 vulnerabilities (flatted, glob, minimatch, rollup, ajv, js-yaml, lodash, brace-expansion, @tootallnate/once, tar chains)
- 3 remaining moderate vulns in `yauzl → extract-zip → electron` chain (build-time only, unfixable without breaking Electron version)

### Added
- `AUDIT_REPORT.md` — Full forensic audit report with all findings, fixes, and verification results

## [1.2.3] - 2026-03-14 - Full Compliance Audit

### Changed
- `no-sandbox` switch injected into Linux Chromium flags block in `src/main.js`
- Ports rotated to new random values: ELECTRON_DEBUG_PORT=62784, ELECTRON_INSPECT_PORT=50799, DEV_SERVER_PORT=55736 across all three run scripts
- `.nvmrc` bumped to Node LTS 24
- `author` field in `package.json` normalized to "J. Michaels"
- AGENTS.md synced to match CLAUDE.md content
- CLAUDE.md updated to reference Node 24

### Added
- `tests/` and `legacy/` directories with `.gitkeep` files
...
### Documentation
- Full 27-file documentation standard applied across the repository
- **Root:** README.md rewritten to reflect v1.2.2, correct repo URL (sanchez314c/mac-spoofer), actual window dimensions (1100x850), and accurate architecture description
- **Root:** CONTRIBUTING.md rewritten with real architecture table, actual security requirements, and correct IPC patterns
- **Root:** CODE_OF_CONDUCT.md — placeholder contact replaced with GitHub issues URL
- **Root:** SECURITY.md rewritten with correct supported versions, actual webPreferences snippet, AuthHandler escalation table, and `experimentalFeatures` prohibition
- **Root:** LICENSE — copyright year corrected to 2026
- **Root:** VERSION_MAP.md — updated with full v1.0.0 through v1.2.2 history
- **Root:** CLAUDE.md — rewritten: Node 22 (was 18), `experimentalFeatures` hard prohibition added, complete API surface documented, Linux flags documented, removed references to nonexistent subdirectories
- **Root:** AGENTS.md — differentiated from CLAUDE.md; now focuses on automation/CI context with process boundary table and IPC channel inventory
- **.github/ISSUE_TEMPLATE/bug_report.md** — created with Python script test command and DevTools instructions
- **.github/ISSUE_TEMPLATE/feature_request.md** — created
- **.github/PULL_REQUEST_TEMPLATE.md** — rewritten with actual test checklist and `experimentalFeatures` security check
- **docs/README.md** — created (replaced DOCUMENTATION_INDEX.md); indexes all 15 docs files with source file overview
- **docs/ARCHITECTURE.md** — fully rewritten from source: accurate three-layer diagram, actual method names, UI layout ASCII, design token table, `backdrop-filter` rationale
- **docs/API.md** — fully rewritten: correct `window.electronAPI.*` surface (not `window.api.*`), actual Python CLI flags, IPC channel reference table
- **docs/TECHSTACK.md** — rewritten from package.json: Electron 39, React/TS/Vite declared-but-unused note, Python stdlib module table
- **docs/INSTALLATION.md** — rewritten: removed fabricated homebrew/choco/snap entries, correct git clone URL, Linux transparency requirements, actual Python CLI verification command
- **docs/DEVELOPMENT.md** — git clone URL corrected
- **docs/DEPLOYMENT.md** — git clone URL and publish owner corrected
- **docs/TROUBLESHOOTING.md** — removed fabricated Python flags (`--check-permissions`, `--info`, `--generate`, `--validate`); replaced with actual `--list`, `--status`, `-i/--random/--yes`, `--restore` flags; updated support URL
- **docs/FAQ.md** — all jasonpaulmichaels/MAC_Spoofer URLs corrected; DOCUMENTATION_INDEX.md link replaced with README.md
- **docs/WORKFLOW.md** — git clone URL corrected
- **docs/QUICK_START.md** — all wrong URLs corrected; fabricated Python CLI flags replaced with actual flags
- **docs/LEARNINGS.md** — added Challenge 0 (`experimentalFeatures` Linux IPC breakage) and Challenge 3b (Linux transparency/backdrop-filter) documenting the two most significant technical discoveries
- Archived: docs/DOCUMENTATION_INDEX.md, docs/CONTRIBUTING.md, docs/CODE_OF_CONDUCT.md, docs/SECURITY.md (all moved to AI-Pre-Trash; governance files now live at root only)
