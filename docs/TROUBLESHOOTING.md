...
# Show spoof status for all interfaces
...
# Show spoof status (current vs original MAC per interface)
python3 src/universal_mac_spoof.py --status

# Spoof an interface with a random locally administered MAC
sudo python3 src/universal_mac_spoof.py -i en0 --random --yes

# Restore original MAC for an interface
...
**Still having issues? Check the [GitHub Issues](https://github.com/sanchez314c/mac-spoofer/issues) or create a new issue with detailed information.**