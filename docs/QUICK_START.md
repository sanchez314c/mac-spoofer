...
1. **Download** for your platform:
   - [macOS Intel](https://github.com/sanchez314c/mac-spoofer/releases/download/v1.0.0/MAC-Address-Spoofer-1.0.0.dmg)
   - [macOS Apple Silicon](https://github.com/sanchez314c/mac-spoofer/releases/download/v1.0.0/MAC-Address-Spoofer-1.0.0-arm64.dmg)
   - [Windows](https://github.com/sanchez314c/mac-spoofer/releases/download/v1.0.0/MAC-Address-Spoofer-Setup-1.0.0.exe)
   - [Linux](https://github.com/sanchez314c/mac-spoofer/releases/download/v1.0.0/MAC-Address-Spoofer-1.0.0.AppImage)
...
# Show spoof status (current vs original MAC)
python3 src/universal_mac_spoof.py --status

# Spoof with a random locally administered MAC (requires sudo/admin)
sudo python3 src/universal_mac_spoof.py -i en0 --random --yes

# Spoof with a specific MAC
sudo python3 src/universal_mac_spoof.py -i en0 -m 02:AB:CD:EF:12:34 --yes

# Restore original MAC
sudo python3 src/universal_mac_spoof.py --restore en0 --yes
```

The `--yes` flag skips interactive confirmation prompts — required when calling from scripts (and what the GUI always passes).

### Automation Script

```bash
#!/bin/bash
# Spoof en0 with a new random MAC
sudo python3 src/universal_mac_spoof.py -i en0 --random --yes
```
...
- **[GitHub Issues](https://github.com/sanchez314c/mac-spoofer/issues)**
...
**Ready to start? [Download now](https://github.com/sanchez314c/mac-spoofer/releases) or [build from source](INSTALLATION.md).**
