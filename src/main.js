  ...
  setupIpcHandlers() {
    ...
    ipcMain.handle(
      ...
      async (event, { interface: interfaceName, mac, random }) => {
        try {
          // Validate interface name: only allow alphanumeric, dash, underscore, dot (covers en0, eth0, wlan0, etc.)
          if (typeof interfaceName !== 'string' || !/^[\w\-.]{1,64}$/.test(interfaceName)) {
            return { success: false, output: '', error: 'Invalid interface name' };
          }
          // Validate MAC if provided
          if (mac !== null && mac !== undefined) {
            if (typeof mac !== 'string' || !/^([0-9A-Fa-f]{2}[:-]){5}[0-9A-Fa-f]{2}$/.test(mac)) {
              return { success: false, output: '', error: 'Invalid MAC address format' };
            }
          }
    ...
    // Open external link — only allow https:// and http:// URLs to prevent protocol injection
