...
class MACSpooferApp {
    ...
    setupIpcHandlers() {
        ...
        // Spoof MAC address
        ipcMain.handle('spoof-mac', async (event, { interface: interfaceName, mac, random }) => {
            try {
                const pythonCmd = await this.findPythonCommand();
                const pythonScript = this.getPythonScript();
        ...
        ipcMain.handle('test-authentication', async () => {
            try {
                ...
                const result = await this.authHandler.executeWithAuth(testCommand, testArgs);
                return result;
            } catch (error) {
                return {
                    success: false,
        ...
        for (const line of lines) {
            ...
            if (trimmed.startsWith('Interface:')) {
                ...
                if (trimmed.startsWith('Current MAC:')) {
                    ...
                    currentInterface.currentMac = mac === 'N/A' ? null : mac.toUpperCase();
                } else if (trimmed.startsWith('Original MAC:')) {
                    const mac = trimmed.replace('Original MAC:', '').trim();
                    currentInterface.originalMac = mac === 'Unknown' || mac === 'N/A' ? null : mac.toUpperCase();
                } else if (trimmed.startsWith('Spoofed:')) {
                    currentInterface.spoofed = trimmed.replace('Spoofed:', '').trim() === 'Yes';
                }
            }
        }
