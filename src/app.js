// HTML escape helper — prevents XSS when inserting data-origin strings into innerHTML
...
class MACSpooferUI {
  ...
  setupEventListeners() {
    ...
    // Title bar Help / Settings button
  ...
  renderInterfaces() {
    ...
    interfaceList.innerHTML = this.interfaces
      .map(
        iface => `
          <div class="interface-item" data-interface="${escHtml(iface.name)}">
            <div class="interface-info">
              <h3>${escHtml(iface.name)}</h3>
              <p>${escHtml(iface.mac) || 'No MAC address'}</p>
            </div>
    ...
    this.updateStatusBar(this.interfaces.length);
  }

  updateStatusBar(count) {
    const countEl = document.getElementById('statusBarCount');
    const textEl = document.getElementById('statusBarText');
    const dotEl = document.getElementById('statusBarDot');

    if (countEl) {
      countEl.textContent = `${count} interface${count !== 1 ? 's' : ''}`;
    }
    if (textEl) {
      textEl.textContent = 'Status: Ready';
    }
    if (dotEl) {
      dotEl.classList.toggle('offline', count === 0);
    }
  }
  ...
  updateSelectedInterfaceDisplay() {
    ...
    selectedInterface.innerHTML = `
      <div class="selected-info">
        <h3>${escHtml(this.selectedInterface.name)}</h3>
        <div class="selected-details">
          <span class="label">Current MAC:</span>
          <span class="value">${escHtml(this.selectedInterface.mac) || 'N/A'}</span>
          ...
          ${
            statusInfo?.originalMac
              ...
              <span class="value">${escHtml(statusInfo.originalMac)}</span>
          `
              : ''
          }
        </div>
        ...
        status => `
          <div class="status-item ${status.spoofed ? 'spoofed' : 'original'}">
            ...
            <div class="status-details">
              <span class="label">Current MAC:</span>
              <span class="value">${escHtml(status.currentMac) || 'N/A'}</span>
              <span class="label">Original MAC:</span>
              <span class="value">${escHtml(status.originalMac) || 'Unknown'}</span>
              <span class="label">Status:</span>
