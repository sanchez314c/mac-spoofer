...
class AuthHandler {
  ...
  async executeWindows(command, args, options) {
    ...
    if (isAdmin) {
      ...
      return new Promise(resolve => {
        // Escape double quotes for PowerShell string interpolation
        const psEscape = str => String(str).replace(/"/g, '""');
        const escapedCommand = psEscape(command);
        const escapedArgs = args
          .map(arg => `"${psEscape(arg)}"`)
          .join(', ');
        const psCommand = `Start-Process -FilePath "${escapedCommand}" -ArgumentList ${escapedArgs} -Verb RunAs -Wait -PassThru | ForEach-Object { $_.ExitCode }`;
  ...
  executeMacOS(command, args, options) {
    ...
    return new Promise(resolve => {
      // Shell-quote each argument: wrap in single quotes, escape any embedded single quotes
      ...
      const fullCommand = `${quotedCommand} ${escapedArgs}`;

      // Escape backslashes and double-quotes so the AppleScript string boundary is never broken
      const escapedForAppleScript = fullCommand
        .replace(/\\/g, '\\\\')
        .replace(/"/g, '\\"');
