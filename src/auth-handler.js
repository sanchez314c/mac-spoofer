...
class AuthHandler {
    ...
    executeWindows(command, args, options) {
    ...
    executeLinux(command, args, options) {
        // On Linux, try pkexec first, then gksudo, then sudo
        return new Promise(async (resolve) => {
            // Try pkexec (PolicyKit)
            const pkexecResult = await this.tryLinuxAuth('pkexec', command, args, options);
            if (pkexecResult.success || !pkexecResult.notFound) {
                return resolve(pkexecResult);
            }

            // Try gksudo (GNOME)
            const gksudoResult = await this.tryLinuxAuth('gksudo', command, args, options);
            if (gksudoResult.success || !gksudoResult.notFound) {
                return resolve(gksudoResult);
            }

            // Try kdesudo (KDE)
            const kdesudoResult = await this.tryLinuxAuth('kdesudo', command, args, options);
            if (kdesudoResult.success || !kdesudoResult.notFound) {
                return resolve(kdesudoResult);
            }

            // Fallback to regular sudo
            const sudoResult = await this.tryLinuxAuth('sudo', command, args, options);
            resolve(sudoResult);
        });
    }
    ...
    tryLinuxAuth(authCommand, command, args, options) {
        return new Promise((resolve) => {
            const authArgs = authCommand === 'sudo' ? ['-S', command, ...args] : [command, ...args];
